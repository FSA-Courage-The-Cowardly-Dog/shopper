                    ...
                    <div className='cart-display'>
                        ...
                        <ul>
                            {cart.lineItems.map((lineItem,idx) => {
                                return(
                                    <li key={idx}>
                                        <span><Link to={`/singleproduct/${lineItem.productId}`}>{lineItem.product.name}</Link>, Unit Price: {lineItem.product.price}, Qty:  <input type='number' defaultValue={lineItem.quantity} onChange={updateQtyForUserCart(lineItem.id)}/></span>
                                        <button 
                            ...
                            {Object.entries(cart).map((pair,idx) => {
                                return(
                                    <li key={idx}>
                                        ...
                                        */}
                                        <span><Link to={`/singleproduct/${pair[0]}`}>{pair[1].name}</Link>, Unit Price: {pair[1].price} Qty:  <input type='number' defaultValue={pair[1].qty} min='1' onChange={updateQtyForLocalCart(pair[0])}/></span>
                                        <button 
                                            className='delete-from-cart' 
                                            type='click' 
